...
public class AstronomicalBodyHelper {
	/**
	 * Returns the size multiplier for a body at the input distance, relative to
	 * either 1AU or the moon's orbital distance, depending on parent body
	 *
	...
	/**
	 * Returns the orbital period for a body at a given distance around its parent
	 * planet
	 *
	 * @param orbitalDistance the distance from the parent body
	...
	public static double getMoonOrbitalPeriod(float orbitalDistance, float planetaryMass) {
		...
		// The same a the function for planets, but since gravity is directly correlated
		// with mass uses the gravity of the plant for mass
		return 8d * Math.pow(Math.pow((orbitalDistance / 100d), 3) / planetaryMass, 0.5d);
	}

	/**
	 * Returns the orbital theta for a body at a given distance around its star, at
	 * this current moment
	 *
	 * @param orbitalDistance the distance from the parent body
	 * @param solarSize       the size of the sun in question
	...
	public static double getOrbitalTheta(int orbitalDistance, float solarSize) {
		...
		return ((AdvancedRocketry.proxy.getWorldTimeUniversal(0) % (24000d * orbitalPeriod)) / (24000d * orbitalPeriod))
				* (2d * Math.PI);
	}

	/**
	 * Returns the orbital theta for a body at a given distance around its parent
	 * planet, at this current moment
	 *
	 * @param orbitalDistance               the distance from the parent body
	 * @param parentGravitationalMultiplier the size of the parent planet in
	 *                                      question
	...
	public static double getMoonOrbitalTheta(int orbitalDistance, float parentGravitationalMultiplier) {
		// Because the function is still in AU and solar mass, some correctional factors
		// to convert to those units
	...
	/**
	 * Returns the visual orbital theta for a body at a given distance around its
	 * parent planet, at this current moment, as a value from 0 - 360
	 *
	 * @param rotationalPeriod              the rotational period of the moon we are
	 *                                      rendering from
	 * @param orbitalDistance               the distance from the parent body
	 * @param parentGravitationalMultiplier the distance from the parent body
	 * @param currentOrbitalTheta           the orbital theta of the moon we are
	 *                                      rendering from
	 * @param baseOrbitalTheta              the base orbital theta of the planet in
	 *                                      question
	 * @return the current angle around the planet normalized 0 - 360, for GL calls
	 */
	public static float getParentPlanetThetaFromMoon(int rotationalPeriod, int orbitalDistance,
			float parentGravitationalMultiplier, double currentOrbitalTheta, double baseOrbitalTheta) {
		// Convert from radians to degrees for easier math
		float degreeOrbitalTheta = (float) (currentOrbitalTheta * 180 / Math.PI);
		// Computer the number of rotations per revolution and use that for how fast the
		// planet would seem to orbit from the moon
		// Planet will not move at all if it is tidally locked
		float planetPositionTheta = (((float) (AstronomicalBodyHelper.getMoonOrbitalPeriod(orbitalDistance,
				parentGravitationalMultiplier) * 24000) / rotationalPeriod) - 1) * degreeOrbitalTheta;
	...
	public static int getAverageTemperature(StellarBody star, int orbitalDistance, int atmPressure) {
		...
		double averageWithoutAtmosphere = starSurfaceTemperature
				* Math.pow(starRadius / (2 * planetaryOrbitalRadius), 0.5) * Math.pow((1f - 0.3f), 0.25);
		// Slightly kludgey solution that works out mostly for Venus and well for Earth,
	...
	public static double getStellarBrightness(StellarBody star, int orbitalDistance) {
		// Normal stars are 1.0 times this value, black holes with accretion discs emit
		// less and so modify it
		...
		// There's no real easy way to get the light emitted by an accretion disc, so
		// this substitutes
		if (blackHole)
			lightMultiplier *= 0.25;
		// Returns ratio compared to a planet at 1 AU for Sol, because the other values
		// in AR are normalized,
		// and this works fairly well for hooking into with other mod's solar panels &
		// such
		return (lightMultiplier * ((Math.pow(star.getSize(), 2) * Math.pow(normalizedStarTemperature, 4))
	...
	public static double getPlanetaryLightLevelMultiplier(double stellarBrightnessMultiplier) {
		...
		// Returns the brightness visible to the eye, compared to the actual flux - this
		// is a factor of ~1.5x for every 2x increase in luminosity
		// This is used for planetary light levels, as those would be eyesight based
		// unlike the stellar brightness or similar
